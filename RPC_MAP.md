...
| End Season | Admin Dashboard *(admin-only — gated; use `armored_archer/admin_trigger_season_event` with `action:"end_season"` for live ops)* | `rpcEndSeason()` in `season_system.ts` — wrapped with `withAdminGuard` | `seasons` storage (custom collection) + `leaderboard` (Nakama) | `/rpc/armored_archer/end_season` |
...
- Authorized callers are configured via the `ADMIN_USER_IDS` environment variable (comma-separated Nakama user ids in strict UUID v4 form, RFC 4122 — see `backend/.env.example` and ADR-0006). Unset/empty rejects every caller (fail-closed), and rejections are audit-logged as `admin_rpc_access_denied`.
