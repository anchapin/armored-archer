...
func _on_models_http_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if result == 0:
		...
		# Only show as debug output, not an error - this is expected if no AI server is configured
		if OS.is_debug_build():
			print_debug("AI Assistant Hub: Could not fetch models (Server URL not configured or unreachable)")
