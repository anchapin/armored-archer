...
export const createMockNakama = (): Runtime.Nakama => {
  ...
  storageReadMock.mockReturnValue = function (this: any, value: any) {
    ...
    if (
      Array.isArray(value) &&
      value.length > 0 &&
      value[0]?.collection === 'player_inventory' &&
      value[0]?.value
    ) {
  ...
  const dbQueryMock = jest.fn((query: string, params?: unknown[]) => {
    ...
    if (
      query.includes('loadout') &&
      (query.includes('UPDATE') || query.includes('ON CONFLICT')) &&
      params
    ) {
    ...
    if (
      query.includes('inventory_items') &&
      query.includes('SELECT') &&
      query.includes('gear_type')
    ) {
      if (testStorage.has(inventoryKey)) {
        try {
          const inventoryValue = testStorage.get(inventoryKey);
    ...
    if (query.includes('boss_defeats')) {
      ...
      const bossDefeatsData: {
        [bossId: string]: { defeat_count: number; first_defeated_at: number };
      } = {};
    ...
    if (query.includes('unlocked_modifier_pools')) {
      ...
      if (
        query.includes('SELECT modifier_id') &&
        query.includes('WHERE user_id =') &&
        query.includes('AND modifier_id =') &&
        params &&
        params.length >= 2
      ) {
      ...
      if (
        query.includes('SELECT modifier_id') &&
        query.includes('FROM unlocked_modifier_pools') &&
