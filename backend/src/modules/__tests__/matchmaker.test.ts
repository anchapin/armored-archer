...
describe("matchmaker", () => {
  ...
  describe("rpcCreateMatch", () => {
    it("should create a match with target opponent", () => {
      ...
      const targetPlayerStats = createPlayerStats({ level: 5, stats: { attack: 18, defense: 14, dodge: 9, crit_rate: 7 } });

      mockNk.storageRead = jest.fn((objects) => {
        if (objects[0].key === "test-user-123") {
          return [{ collection: "player_stats", key: "test-user-123", userId: "test-user-123", value: JSON.stringify(playerStats) }];
        } else if (objects[0].key === "target-user") {
          return [{ collection: "player_stats", key: "target-user", userId: "target-user", value: JSON.stringify(targetPlayerStats) }];
        }
        return [];
      });
    ...
    it("should allow punch-up when flag set", () => {
      ...
      const payload = JSON.stringify({ match_type: "ranked", target_opponent_id: "target-user", is_punch_up: true });
      const result = rpcCreateMatch(mockCtx, mockLogger, mockNk, payload);
      const parsed = JSON.parse(result);
  ...
  describe("rpcAcceptMatch", () => {
    it("should accept pending match successfully", () => {
      ...
      const playerStats = { level: 5, xp: 500, stats: { attack: 20, defense: 15, dodge: 10, crit_rate: 8 } };
  ...
  describe("rpcListMatches", () => {
    it("should list pending matches excluding own", () => {
      ...
      mockNk.storageList = jest.fn(() => matches.map(m => ({ collection: "pvp_matches", key: m.match_id, value: JSON.stringify(m) })));
    ...
    it("should filter by match_type", () => {
      ...
      mockNk.storageRead = jest.fn(() => [{ collection: "player_stats", key: "test-user-123", value: JSON.stringify(playerStats) }]);
      mockNk.storageList = jest.fn(() => matches.map(m => ({ collection: "pvp_matches", key: m.match_id, value: JSON.stringify(m) })));
    ...
    it("should apply limit correctly", () => {
      const playerStats = { level: 5, xp: 500, stats: { attack: 20, defense: 15, dodge: 10, crit_rate: 8 } };
      const matches = Array.from({ length: 5 }, (_, i) => createMatch({ match_id: `m${i}`, created_at: Date.now() - i * 1000 }));
  ...
  describe("rpcGetPlayerRank", () => {
    it("should return player rank, level, and xp", () => {
      const playerStats = { level: 7, xp: 1250, stats: { attack: 25, defense: 20, dodge: 15, crit_rate: 12 } };
      mockNk.storageRead = jest.fn(() => [{
        collection: "player_stats",
        key: "test-user-123",
        value: JSON.stringify(playerStats)
