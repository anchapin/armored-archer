import { batchGetPlayerStats, getPlayerStatsWithCache, invalidatePlayerStatsCache } from "../db_optimizer";
...
describe("db_optimizer", () => {
  ...
  describe("batchGetPlayerStats", () => {
    ...
    it("fetches from storage for uncached users", () => {
      ...
      const raw = [{ collection: "player_stats", key: "u1", userId: "u1", value: '{"level":5,"stats":{"attack":10}}' }];
      mockNk.storageRead.mockReturnValue(raw);
      (safeParse as jest.Mock).mockReturnValue({ success: true, data: { level: 5, stats: { attack: 10 } } });

      const result = batchGetPlayerStats(mockNk, ["u1"], mockLogger);
      expect(result.get("u1")).toEqual({ level: 5, stats: { attack: 10 } });
      expect(mockCache.set).toHaveBeenCalledWith("player_stats", "u1", { level: 5, stats: { attack: 10 } });
    });
  ...
  describe("getPlayerStatsWithCache", () => {
    ...
    it("fetches and caches if not in cache", () => {
      ...
      (safeParse as jest.Mock).mockReturnValue({ success: true, data: { level: 4, stats: { attack: 10 } } });

      const stats = getPlayerStatsWithCache(mockNk, "u1", mockLogger);
      expect(stats.level).toBe(4);
      expect(mockCache.set).toHaveBeenCalledWith("player_stats", "u1", { level: 4, stats: { attack: 10 } });
    });
