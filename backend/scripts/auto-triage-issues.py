...
class TriageResult:
    ...
    comment_suggestion: Optional[str] = None
...
class AutoTriage:
    ...
    def __init__(self, github_token: Optional[str] = None, repo: str = "armored-archer", owner: str = None):
        """
    ...
    def _detect_category(self, content: str) -> tuple[Optional[str], float]:
        """
    ...
    def apply_labels(self, issue_number: int, labels: list[str]) -> bool:
        ...
        """
        if not self.repository:
            print(f"⚠️  Cannot apply labels: No GitHub connection")
            return False
    ...
    def add_comment(self, issue_number: int, comment: str) -> bool:
        ...
        """
        if not self.repository:
            print(f"⚠️  Cannot add comment: No GitHub connection")
            return False
    ...
    def send_slack_alert(self, result: TriageResult) -> bool:
        ...
        if not slack_webhook:
            print(f"⚠️  No Slack webhook URL configured")
            return False
...
def print_triage_result(result: TriageResult, verbose: bool = False):
    ...
    if result.should_alert_slack:
        print(f"   🚨 Slack alert will be sent")
