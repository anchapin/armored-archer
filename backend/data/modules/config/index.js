...
function loadEnvironment() {
    ...
    var envFiles = [".env.".concat(nodeEnv), '.env', ".env.".concat(nodeEnv, ".local")];
    try {
        for (var envFiles_1 = tslib_1.__values(envFiles), envFiles_1_1 = envFiles_1.next(); !envFiles_1_1.done; envFiles_1_1 = envFiles_1.next()) {
            var file = envFiles_1_1.value;
            var envPath = path.join(process.cwd(), file);
            if (fs.existsSync(envPath)) {
                var envContent = fs.readFileSync(envPath, 'utf-8');
                try {
                    for (var _c = (e_2 = void 0, tslib_1.__values(envContent.split('\n'))), _d = _c.next(); !_d.done; _d = _c.next()) {
                        var line = _d.value;
        ...
        try {
            if (envFiles_1_1 && !envFiles_1_1.done && (_a = envFiles_1.return)) _a.call(envFiles_1);
        }
        finally { if (e_1) throw e_1.error; }
    }
}
loadEnvironment();
function parseDatabaseAddress(address) {
    // Support both formats:
    // - postgres://user:password@host:port/database
    // - user:password@host:port/database
    var dbRegex = /^(?:postgres:\/\/)?(\w+):([^@]+)@([^:]+):(\d+)\/(\w+)$/;
    var match = address.match(dbRegex);
    if (!match) {
        return {
            address: address,
    ...
    var _a = tslib_1.__read(match, 6), user = _a[1], password = _a[2], host = _a[3], port = _a[4], database = _a[5];
    return {
        address: address,
        host: host,
...
var dbAddress = process.env.DATABASE_ADDRESS || process.env.NAKAMA_DATABASE_ADDRESS || '';
var config = {
    ...
    logger: {
        ...
        additionalSensitiveFields: process.env.LOG_SCRUB_ADDITIONAL_FIELDS
            ? process.env.LOG_SCRUB_ADDITIONAL_FIELDS.split(',').map(function (f) { return f.trim(); })
            : undefined,
        ...
        scrubOutputs: process.env.LOG_SCRUB_OUTPUTS
            ? process.env.LOG_SCRUB_OUTPUTS.split(',').map(function (o) { return o.trim(); })
            : undefined,
    },
    match: {
    ...
    tracing: {
        ...
        instrumentations: process.env.TRACING_INSTRUMENTATIONS
            ? process.env.TRACING_INSTRUMENTATIONS.split(',').map(function (i) { return i.trim(); })
            : ['http', 'express', 'pg'],
    },
    alerting: {
        ...
        healthAlerts: {
            ...
            dbConnectionsWarningPercent: parseInt(process.env.ALERT_DB_CONNECTIONS_WARNING_PERCENT || '70', 10),
            dbConnectionsCriticalPercent: parseInt(process.env.ALERT_DB_CONNECTIONS_CRITICAL_PERCENT || '90', 10),
        ...
        metricAlerts: {
            ...
            activeConnectionsCritical: parseInt(process.env.ALERT_ACTIVE_CONNECTIONS_CRITICAL || '2000', 10),
            ...
            matchWaitTimeCriticalSec: parseInt(process.env.ALERT_MATCH_WAIT_TIME_CRITICAL_SEC || '180', 10),
    ...
    datadog: process.env.DATADOG_ENABLED === 'true'
        ? {
            ...
            tags: tslib_1.__assign({ environment: process.env.NODE_ENV || 'development', service: 'armored-archer-backend' }, (process.env.DATADOG_TAGS ? JSON.parse(process.env.DATADOG_TAGS) : {})),
        }
        : undefined,
};
/**
 ...
 */
function isInvalidConfigValue(value, productionOnly) {
    if (productionOnly === void 0) { productionOnly = false; }
    if (!value)
        return true;
    var strValue = String(value);
    var isDefault = strValue === 'defaultkey' ||
        strValue === 'default-token-key' ||
        strValue === 'default-refresh-key';
    if (isDefault && productionOnly)
        return true;
    return false;
}
/**
...
function validateSessionKeys() {
    ...
    try {
        for (var requiredKeys_1 = tslib_1.__values(requiredKeys), requiredKeys_1_1 = requiredKeys_1.next(); !requiredKeys_1_1.done; requiredKeys_1_1 = requiredKeys_1.next()) {
            var key = requiredKeys_1_1.value;
            var value = getNestedConfigValue(key);
            if (isInvalidConfigValue(value, isProduction)) {
                errors.push("".concat(key, " must be set in production"));
            }
        }
    }
...
function validateRequiredConfig() {
    ...
    var sessionErrors = validateSessionKeys();
    if (sessionErrors.length > 0) {
        throw new Error("Configuration validation failed:\n".concat(sessionErrors.map(function (e) { return "  - ".concat(e); }).join('\n')));
    }
    // Validate port numbers
    if (!isValidPort(config.server.port)) {
        throw new Error("Invalid server port: ".concat(config.server.port, ". Must be between 1 and 65535"));
    }
    if (!isValidPort(config.server.consolePort)) {
        throw new Error("Invalid console port: ".concat(config.server.consolePort, ". Must be between 1 and 65535"));
    }
    if (!isValidPort(config.database.port)) {
        throw new Error("Invalid database port: ".concat(config.database.port, ". Must be between 1 and 65535"));
    }
}
...
function logConfiguration(logger) {
    ...
    logger.info('Server: host=%s, port=%d, console_port=%d', config.server.host, config.server.port, config.server.consolePort);
    logger.info('Database: host=%s, port=%d, database=%s', config.database.host, config.database.port, config.database.database);
    logger.info('RevenueCat: public_key=%s, webhook_secret_configured=%s', maskSecret(config.revenuecat.publicKey), config.revenuecat.webhookSecret ? 'true' : 'false');
    logger.info('Session: expiry_sec=%d', config.session.expirySec);
    logger.info('Logger: level=%s, format=%s, output=%s, scrubLogs=%s', config.logger.level, config.logger.format, config.logger.output, config.logger.scrubLogs);
    logger.info('Match: allow_host_loopback=%s', config.match.allowHostLoopback);
    logger.info('Metrics: namespace=%s, prometheus_port=%d', config.metrics.namespace, config.metrics.prometheusPort);
    logger.info('Alerting: enabled=%s, default_provider=%s, min_env_level=%s', config.alerting.enabled, config.alerting.defaultProvider, config.alerting.minEnvironmentLevel);
    logger.info('====================');
}
