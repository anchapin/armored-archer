...
exports.registerRpcJoinPool = registerRpcJoinPool;
exports.rpcJoinPool = rpcJoinPool;
exports.registerRpcLeavePool = registerRpcLeavePool;
exports.rpcLeavePool = rpcLeavePool;
exports.registerRpcGetQueueStatus = registerRpcGetQueueStatus;
exports.rpcGetQueueStatus = rpcGetQueueStatus;
exports.processMatchmaking = processMatchmaking;
...
function getPool(nk, mode) {
    ...
    const result = (0, safeParse_1.safeParse)(objects[0].value, null, undefined, 'getPool');
 ...
 */
function savePool(nk, pool, mode) {
    const poolKey = `${POOL_STORAGE_KEY}_${mode}`;
    nk.storageWrite([
        {
            collection: 'matchmaking',
 ...
 */
function calculateBracketSize(waitTime) {
...
function findBestMatch(player, pool) {
    ...
    const candidates = pool.players.filter((p) => p.user_id !== player.user_id && p.mode === player.mode);
 ...
 */
function registerRpcJoinPool(initializer) {
 ...
 */
function rpcJoinPool(ctx, logger, nk, payload) {
    logger.info('Join pool called for user: %s', ctx.userId);
    const validation = (0, validation_1.validatePayload)(validation_1.ZodSchemas.join_pool, payload, 'join_pool');
    if (!validation.success) {
        return (0, validation_1.createValidationErrorResponse)('join_pool', validation.error);
    }
    ...
    logger.info('Player %s joined pool for mode %s with rating %d. Position: %d', ctx.userId, request.mode, request.rating, queuePosition);
    return JSON.stringify({
        success: true,
 ...
 */
function registerRpcLeavePool(initializer) {
 ...
 */
function rpcLeavePool(ctx, logger, nk, payload) {
    logger.info('Leave pool called for user: %s', ctx.userId);
    const validation = (0, validation_1.validatePayload)(validation_1.ZodSchemas.leave_pool, payload, 'leave_pool');
    if (!validation.success) {
        return (0, validation_1.createValidationErrorResponse)('leave_pool', validation.error);
    }
    const request = validation.data;
    // Get current pool
    const pool = getPool(nk, request.mode);
 ...
 */
function registerRpcGetQueueStatus(initializer) {
 ...
 * @returns JSON string with queue position and estimated wait
 */
function rpcGetQueueStatus(ctx, logger, nk, payload) {
    logger.info('Get queue status called for user: %s', ctx.userId);
    const validation = (0, validation_1.validatePayload)(validation_1.ZodSchemas.get_queue_status, payload, 'get_queue_status');
    if (!validation.success) {
        return (0, validation_1.createValidationErrorResponse)('get_queue_status', validation.error);
    }
    const request = validation.data;
    // Get current pool
    const pool = getPool(nk, request.mode);
    ...
    if (matchedPlayer) {
        logger.info('Match found for %s with %s in mode %s', ctx.userId, matchedPlayer.user_id, request.mode);
        // Remove both players from pool
        const filteredPlayers = pool.players.filter((p) => p.user_id !== ctx.userId && p.user_id !== matchedPlayer.user_id);
 ...
 */
function processMatchmaking(logger, nk) {
    const modes = ['1v1', '2v2'];
    for (const mode of modes) {
        ...
        for (let i = 0; i < sortedPlayers.length; i++) {
            ...
            if (match) {
                ...
                pool.players = pool.players.filter((p) => p.user_id !== player.user_id && p.user_id !== match.user_id);
