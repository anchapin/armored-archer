...
function registerRpcRegisterDeviceToken(initializer) {
    initializer.registerRpc('armored_archer_register_device_token', async (ctx, logger, nk, payload) => {
        try {
            ...
            const result = await (0, notifications_1.registerDeviceToken)(nk, ctx.userId, deviceToken, platform, appVersion, fcmToken);
...
function registerRpcRemoveDeviceToken(initializer) {
    initializer.registerRpc('armored_archer_remove_device_token', async (ctx, logger, nk, payload) => {
        try {
            const { deviceToken } = JSON.parse(payload);
            if (!deviceToken) {
                return JSON.stringify({ success: false, error: 'Missing deviceToken' });
            }
            const result = await (0, notifications_1.removeDeviceToken)(nk, deviceToken);
            return JSON.stringify(result);
        }
        catch (error) {
...
function registerRpcGetNotificationPreferences(initializer) {
    ...
    async (ctx, logger, nk, payload) => {
        try {
            const prefs = await (0, notifications_1.getNotificationPreferences)(nk, ctx.userId);
            if (!prefs) {
...
function registerRpcUpdateNotificationPreferences(initializer) {
    initializer.registerRpc('armored_archer_update_notification_preferences', async (ctx, logger, nk, payload) => {
        try {
            ...
            const result = await (0, notifications_1.updateNotificationPreferences)(nk, ctx.userId, preferences);
            return JSON.stringify(result);
        }
        catch (error) {
...
function registerRpcScheduleNotification(initializer) {
    initializer.registerRpc('armored_archer_schedule_notification', async (ctx, logger, nk, payload) => {
        try {
            ...
            const template = (0, notifications_1.getNotificationTemplate)(type);
            const notificationTitle = title || template.title;
            const notificationBody = body || template.body;
            const result = await (0, notifications_1.scheduleNotification)(nk, ctx.userId, type, notificationTitle, notificationBody, new Date(scheduledFor), data || {});
            return JSON.stringify(result);
        }
        catch (error) {
...
function registerRpcCancelNotification(initializer) {
    initializer.registerRpc('armored_archer_cancel_notification', async (ctx, logger, nk, payload) => {
        try {
            const { notificationId } = JSON.parse(payload);
            if (!notificationId) {
                return JSON.stringify({ success: false, error: 'Missing notificationId' });
            }
            const result = await (0, notifications_1.cancelScheduledNotification)(nk, notificationId);
            return JSON.stringify(result);
        }
        catch (error) {
...
function registerRpcGetNotificationStatus(initializer) {
    ...
    async (ctx, logger, nk, payload) => {
        try {
            const initialized = (0, notifications_1.isFirebaseInitialized)();
            const deviceTokens = (await nk.dbQuery(`SELECT COUNT(*) as count FROM device_tokens WHERE user_id = $1`, [ctx.userId]));
            return JSON.stringify({
