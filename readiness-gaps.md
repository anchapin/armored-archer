...
| R4 | DoD verification | #917 → [`docs/dod/MVP_PVE_SLICE_DOD_REPORT_2026.md`](docs/dod/MVP_PVE_SLICE_DOD_REPORT_2026.md) | **Status: 🟡 PARTIAL (2026-08-17).** Headless gates: stack cold-start **PASS**, backend lint **PASS**, backend typecheck **PASS**, backend tests **mostly green** (3377/3414 — 5 IAP suites fail: store / revenuecat_webhook / restore_purchases / alerting / config), Godot tests **mostly green** (1044/1050 — 4 enemy-refactor suites fail post #910: `test_base_enemy.gd`, `test_enemy_spawner.gd`, `test_melee_enemy.gd`, plus `test_hit_reactions.gd` parse error). Auth fast-fail UX (#908) structurally verified (bounded 12s, 3 retries, `/v2/health` gate). Android emulator/device checks (happy path, feel pass, 60 FPS) **DEFERRED to human-run** — no Android toolchain on the Linux host. iOS explicitly out of scope (no macOS toolchain). GitHub Actions CI gate out of service (#855). Do **not** flip `MVP_PVE_ONLY` until the 9 test-file cleanup + Android human-run close. |
