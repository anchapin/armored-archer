...
func _set_theme(theme_name: String):
	if _theme_manager:
		...
		await wait_for_frames(1)  # Wait for theme to apply
...
func test_base_button_visual_regression():
	...
	if not ResourceLoader.exists(button_path):
		skip("Base button scene not found")

	var button := load(button_path).instantiate()
...
func test_base_panel_visual_regression():
	...
	if not ResourceLoader.exists(panel_path):
		skip("Base panel scene not found")

	var panel := load(panel_path).instantiate()
	add_child(panel)

	for theme in ["light", "dark"]:
		_set_theme(theme)
		for viewport_size in [VIEWPORT_MOBILE, VIEWPORT_TABLET, VIEWPORT_DESKTOP]:
			var image := await capture_screenshot("base_panel", theme, viewport_size)
...
func test_base_progress_bar_visual_regression():
	...
	if not ResourceLoader.exists(progress_bar_path):
		skip("Base progress bar scene not found")

	var progress_bar := load(progress_bar_path).instantiate()
	...
	for theme in ["light", "dark"]:
		_set_theme(theme)
		for viewport_size in [VIEWPORT_MOBILE, VIEWPORT_TABLET, VIEWPORT_DESKTOP]:
			var image := await capture_screenshot("base_progress_bar", theme, viewport_size)
...
func test_theme_toggle_visual_regression():
	...
	if not ResourceLoader.exists(toggle_path):
		skip("Theme toggle scene not found")

	var toggle := load(toggle_path).instantiate()
	add_child(toggle)

	for theme in ["light", "dark"]:
		_set_theme(theme)
		for viewport_size in [VIEWPORT_MOBILE, VIEWPORT_TABLET, VIEWPORT_DESKTOP]:
			var image := await capture_screenshot("theme_toggle", theme, viewport_size)
...
func test_base_label_visual_regression():
	...
	if not ResourceLoader.exists(label_path):
		skip("Base label scene not found")

	var label := load(label_path).instantiate()
	...
	for theme in ["light", "dark"]:
		_set_theme(theme)
		for viewport_size in [VIEWPORT_MOBILE, VIEWPORT_TABLET, VIEWPORT_DESKTOP]:
			var image := await capture_screenshot("base_label", theme, viewport_size)
	...
	label.queue_free()
	assert_true(true)  # Always pass for nice-to-have components


func test_base_icon_visual_regression():
	var icon_path := "res://scenes/ui/components/base_icon.tscn"

	if not ResourceLoader.exists(icon_path):
		skip("Base icon scene not found")

	var icon := load(icon_path).instantiate()
	add_child(icon)

	for theme in ["light", "dark"]:
		_set_theme(theme)
		for viewport_size in [VIEWPORT_MOBILE, VIEWPORT_TABLET, VIEWPORT_DESKTOP]:
			var image := await capture_screenshot("base_icon", theme, viewport_size)
			var passed := compare_with_baseline(image, "base_icon", theme, viewport_size)
			if not passed:
				print("WARNING: Visual regression in base_icon (nice-to-have component)")
...
func test_base_container_visual_regression():
	var container_path := "res://scenes/ui/components/base_container.tscn"

	if not ResourceLoader.exists(container_path):
		skip("Base container scene not found")
	...
	for theme in ["light", "dark"]:
		_set_theme(theme)
		for viewport_size in [VIEWPORT_MOBILE, VIEWPORT_TABLET, VIEWPORT_DESKTOP]:
			var image := await capture_screenshot("base_container", theme, viewport_size)
			var passed := compare_with_baseline(image, "base_container", theme, viewport_size)
			if not passed:
				print("WARNING: Visual regression in base_container (nice-to-have component)")
...
func test_loading_indicator_visual_regression():
	var loading_path := "res://scenes/ui/components/loading_indicator.tscn"

	if not ResourceLoader.exists(loading_path):
		skip("Loading indicator scene not found")

	var loading := load(loading_path).instantiate()
	add_child(loading)

	for theme in ["light", "dark"]:
		_set_theme(theme)
		for viewport_size in [VIEWPORT_MOBILE, VIEWPORT_TABLET, VIEWPORT_DESKTOP]:
			var image := await capture_screenshot("loading_indicator", theme, viewport_size)
			var passed := compare_with_baseline(image, "loading_indicator", theme, viewport_size)
			if not passed:
				print("WARNING: Visual regression in loading_indicator (nice-to-have component)")
