...
# Issue #1361 follow-up: InventoryManager autoload tests are entirely
# mocked via `_inv.set("NetworkManager", _mock_network)`, but the production
# code calls `NetworkManager.send_rpc(...)` (autoload singleton), not the
# mock. The test suite has been failing for this reason since the deprecation
# (see AGENTS.md "InventoryManager is deprecated — use GearManager"). The
# comment header claims "Tests skipped" but pending() only logs — it does
# not actually return. Add a guard so each test early-returns after pending.
var _skip_inv_tests: bool = true
