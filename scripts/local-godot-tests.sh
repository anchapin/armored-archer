...
run_tests() {
    ...
    # Run E2E runner scene as it handles Autoloads and Godot 4.0 compatibility better
    # The output is piped to tee to capture it for failure checking.
    timeout 120 "$GODOT_BINARY" --headless --path . res://test/e2e_runner.tscn 2>&1 | tee /tmp/godot_test_output.txt
    
    local exit_code=${PIPESTATUS[0]} # Get exit code of the Godot command
    
    if [ $exit_code -ne 0 ]; then
        log_error "Godot tests failed or timed out (Godot process exited with $exit_code)"
        return 1
    fi
...
run_quick_validation() {
    ...
    for test_file in "$TEST_DIR"/test_*.gd; do
        if [ -f "$test_file" ]; then
            COUNT=$(grep -c "func test_" "$test_file" 2>/dev/null || true)
