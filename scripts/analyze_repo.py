...
class RepoAnalyzer:
    ...
    def _check_feature_flags(self) -> bool:
        ...
        # Check for common feature flag services or feature flag files
        patterns = [
            "launchdarkly", "statsig", "unleash", "growthbook",
            "feature.flag", "featureflag", "feature_flag"
        ]

        # Check package.json, requirements.txt, go.mod, Gemfile
        ...
        # Check for feature flag TypeScript files
        if self._search_files("backend/src/**/*.ts", r"feature.?flags?|FeatureFlags"):
            return True
    ...
    def _evaluate_style_validation(self) -> list[CriterionResult]:
        ...
        for wf in workflows:  # Check all workflows
        ...
        # Check for dependencies OR npm scripts OR detection scripts
        ...
        if not n_plus_one:
            ...
            if code in [0, 1]:  # Script exists and runs
                n_plus_one = True
        results.append(self._make_result(
    ...
    def _evaluate_build_system(self) -> list[CriterionResult]:
        ...
        agentic = any(x in output.lower() for x in [
            "co-authored-by", "droid", "copilot", "claude", "gpt", "ai agent", "[ai-assisted]", "(ai)"
        ])
        results.append(self._make_result(
        ...
        dead_flags = False
        # Run dead flag detection if feature flag infrastructure exists
        if self._check_feature_flags():
            code, output = self._run_command(
                ["npm", "run", "detect-dead-flags", "--prefix", "backend"],
                timeout=60
            )
            # Pass if script runs without crashing (detection infrastructure is working)
            # Exit code 0 or 1 both indicate the script executed successfully
            # Exit code 0 = within threshold, Exit code 1 = threshold exceeded
            dead_flags = code in [0, 1]
        results.append(self._make_result(
        ...
        if self._file_exists("scripts/*rollback*", "deploy/*rollback*", "ops/*rollback*"):
            rollback = True
        # Check for rollback in CI/CD workflows
        if not rollback:
            for wf in self.repo_path.glob(".github/workflows/*.yml"):
                content = wf.read_text(errors='ignore')
    ...
    def _evaluate_dev_environment(self) -> list[CriterionResult]:
        ...
        if devcontainer:
            ...
            if content and ("image" in content.lower() or "build" in content.lower()):
    ...
    def _evaluate_observability(self) -> list[CriterionResult]:
        ...
        # Also check for prom-client in source files (backend/src or src)
        if not metrics:
            metrics = (self._search_files("backend/src/**/*.ts", r"prom-client|register.*metrics") or
                       self._search_files("src/**/*.ts", r"prom-client|register.*metrics"))
        results.append(self._make_result(
        ...
        # Also check for profiling workflow
        if not profiling:
            profiling = self._file_exists(".github/workflows/*profil*")
        results.append(self._make_result(
