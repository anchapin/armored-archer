...
func _ready() -> void:
	...
	if theme_manager:
		_apply_theme()
		if theme_manager.theme_changed.connect(_on_theme_changed) != OK:
			push_warning("CampaignMap: Failed to connect to ThemeManager.theme_changed")
	...
	if CampaignManager.stage_unlocked.connect(_on_stage_unlocked) != OK:
		push_warning("CampaignMap: Failed to connect to stage_unlocked signal")
	if CampaignManager.stage_completed.connect(_on_stage_completed) != OK:
		push_warning("CampaignMap: Failed to connect to stage_completed signal")
	if CampaignManager.campaign_progress_updated.connect(_on_progress_updated) != OK:
		push_warning("CampaignMap: Failed to connect to campaign_progress_updated signal")

	# Connect back button
	if back_button and back_button.pressed.connect(_on_back_button_pressed) != OK:
		push_warning("CampaignMap: Failed to connect BackButton.pressed signal")

	# Connect chapter navigation buttons
	if prev_chapter_button and prev_chapter_button.pressed.connect(_on_prev_chapter_pressed) != OK:
		push_warning("CampaignMap: Failed to connect PrevChapterButton.pressed")
	if next_chapter_button and next_chapter_button.pressed.connect(_on_next_chapter_pressed) != OK:
		push_warning("CampaignMap: Failed to connect NextChapterButton.pressed")
...
func _apply_basic_colors() -> void:
	...
	if prev_chapter_button:
		prev_chapter_button.modulate = button_color
	if next_chapter_button:
		next_chapter_button.modulate = button_color
	if has_node("BackButton"):
		$BackButton.modulate = button_color
...
func _apply_theme() -> void:
	...
	var _is_dark = theme_manager.is_dark_theme()
