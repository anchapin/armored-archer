...
func _update_currency_display() -> void:
	if StoreManager:
		gems_label.text = str(StoreManager.get_gems())
		gold_label.text = str(StoreManager.get_gold())
	elif GemManager:
		gems_label.text = str(GemManager.get_gem_balance())

func _update_product_buttons() -> void:
	var products = StoreManager.get_products()
	
	if products.has(StoreManager.PRODUCT_SMALL_GEMS):
		var product = products[StoreManager.PRODUCT_SMALL_GEMS]
		small_gems_button.text = "%s\n$0.99" % product.localized_title
	
	if products.has(StoreManager.PRODUCT_MEDIUM_GEMS):
		var product = products[StoreManager.PRODUCT_MEDIUM_GEMS]
		medium_gems_button.text = "%s\n$4.99" % product.localized_title
	
	if products.has(StoreManager.PRODUCT_LARGE_GEMS):
		var product = products[StoreManager.PRODUCT_LARGE_GEMS]
		large_gems_button.text = "%s\n$9.99" % product.localized_title

# --- Purchase Handlers ---
func _on_small_gems_pressed() -> void:
	_initiate_purchase(StoreManager.PRODUCT_SMALL_GEMS)

func _on_medium_gems_pressed() -> void:
	_initiate_purchase(StoreManager.PRODUCT_MEDIUM_GEMS)

func _on_large_gems_pressed() -> void:
	_initiate_purchase(StoreManager.PRODUCT_LARGE_GEMS)
...
func _initiate_purchase(product_id: String) -> void:
	...
	StoreManager.purchase_product(product_id)
...
func _on_currency_updated(gems: int, gold: int) -> void:
	...
	if GemManager:
		var current_balance = GemManager.get_gem_balance()
		if gems != current_balance:
			var diff = gems - current_balance
			if diff > 0:
				GemManager.add_gems(diff)
			elif diff < 0:
				GemManager.remove_gems(-diff)
