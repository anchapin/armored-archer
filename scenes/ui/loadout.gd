...
func _setup_button_connections() -> void:
	var _err1 = back_button.pressed.connect(_on_back_button_pressed)
	var _err2 = inventory_button.pressed.connect(_on_inventory_button_pressed)
...
func _update_stats_display() -> void:
	...
	for slot_key in equipped.keys():
		...
		if not gear_data.is_empty():
			...
			var base_gear_data: Resource = gear_registry.get_base_gear(base_gear_id)
			if base_gear_data:
