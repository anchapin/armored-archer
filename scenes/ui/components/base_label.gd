...
func _apply_type_style() -> void:
	...
	match label_type:
		LabelType.TITLE:
			font_size = DesignTokens.FONT_SIZE_TITLE
			text_color = _get_text_primary_color()
		LabelType.HEADER:
			font_size = DesignTokens.FONT_SIZE_HEADER
			text_color = _get_text_primary_color()
		LabelType.SUBHEADER:
			font_size = DesignTokens.FONT_SIZE_XL
			text_color = _get_text_primary_color()
		LabelType.BODY:
			font_size = DesignTokens.FONT_SIZE_BASE
			text_color = _get_text_primary_color()
		LabelType.CAPTION:
			font_size = DesignTokens.FONT_SIZE_SM
			text_color = _get_text_secondary_color()
		LabelType.OVERLINE:
			font_size = DesignTokens.FONT_SIZE_XS
			text_color = _get_text_secondary_color()
		LabelType.BUTTON:
			font_size = DesignTokens.FONT_SIZE_BASE
			text_color = _get_text_primary_color()
		LabelType.NUMERIC:
			font_size = DesignTokens.FONT_SIZE_LG
			text_color = _get_text_primary_color()
		_:
			font_size = DesignTokens.FONT_SIZE_BASE
			text_color = _get_text_primary_color()
...
func _get_text_primary_color() -> Color:
	return DesignTokens.COLOR_TEXT_PRIMARY_DARK if _is_dark_theme else DesignTokens.COLOR_TEXT_PRIMARY_LIGHT

func _get_text_secondary_color() -> Color:
	return DesignTokens.COLOR_TEXT_SECONDARY_DARK if _is_dark_theme else DesignTokens.COLOR_TEXT_SECONDARY_LIGHT
